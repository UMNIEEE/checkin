...
// Returns if should crawl is selected. Degraded.
function isCrawl(elemName) {
    if (elemName === null || elemName === undefined)
        return false;
    
    elemName = "#MainContent_" + elemName;
    return ($(elemName).attr("checked") !== undefined);
}
// Performs a U of M person search due to a button click.
function manualSearch() {
    var firstName = $("#MainContent_FirstNameBox").val().trim().toLowerCase();
    globFirstName = firstName;
    var lastName = $("#MainContent_LastNameBox").val().trim().toLowerCase();
    globLastName = lastName;
    var urlPath = "http://www.umn.edu/lookup?SET_INSTITUTION=UMNTC&type=name&CN=" +
                    firstName + "+" + lastName + "&campus=a&role=any";
    crawl(urlPath);
    return false;
}
// Parses the individual person result returned by the crawl and fills in the appropriate form fields.
function parseData() {
    if (globData === null || globData === undefined || globData.length === 0)
        return false;

    // TODO - parse U of M data here
    var regFirst = new RegExp(globFirstName, "i");
    var regLast = new RegExp(globLastName, "i");

    var firstNameInd = globData[0].search(regFirst);
    if (firstNameInd >= 0 && globFirstName.trim() !== "")
        globFirstName = globData[0].substr(firstNameInd, globFirstName.length).trim();
    else
        firstNameInd = -1;
    
    var lastNameInd = globData[0].search(regLast);
    if (lastNameInd >= 0 && globLastName.trim() !== "")
        globLastName = globData[0].substr(lastNameInd, globLastName.length).trim();
    else
        lastNameInd = -1;

    if (firstNameInd < 0) {
        if (lastNameInd >= 0) {
            globFirstName = globData[0].substr(0, lastNameInd - 3).trim();
        }
        else {
            globFirstName = $("#MainContent_FirstNameBox").val().trim();
        }
    }
    if (lastNameInd < 0) {
        if (firstNameInd >= 0) {
            globLastName = globData[0].substr(firstNameInd + 3).trim();
        }
        else {
            globLastName = $("#MainContent_LastNameBox").val().trim();
        }
    }
    ...
    $("#MainContent_FirstNameBox").val(globFirstName.trim());
    $("#MainContent_LastNameBox").val(globLastName.trim());
    $("#MainContent_EmailBox").val(email);
    $("#MainContent_PhoneNumberBox").val(phone);
    $("#MainContent_X500Box").val(x500);
    $("#MainContent_UEmailBox").val(email);
    $("#MainContent_CollegeList").prop("selectedIndex", college);
    $("#MainContent_CampusList").prop("selectedIndex", campus);
    $("#MainContent_EnrollSemList").prop("selectedIndex", sem);
    $("#MainContent_EnrollYearBox").val(year);
    try {
        var addrTable = $("#MainContent_AddrInfoTable").attr("id");
        if (addrTable !== undefined && addrTable !== null) {
            $("#MainContent_StreetAddressBox").val(street);
            $("#MainContent_CityBox").val(city);
            $("#MainContent_ZipcodeBox").val(zip);
            $("#MainContent_CountryList").prop("selectedIndex", 1);
            $("#MainContent_StateList").prop("selectedIndex", state);
        }
    }
    catch (err) {
    ...
    try {
        uPerson = {
            "firstName": globFirstName.trim(),
            "lastName": globLastName.trim(),
            "email": email.trim(),
            "phone": phone.trim(),
            "x500": x500.trim(),
...
function searchResultSelect(row) {
    ...
    $.ajax({
        url: urlPath, type: 'GET', success: function (res) {
            ...
            $('#links').append('<br/>');
            $('#links').append('<input type="submit" name="ctl00$MainContent$crawlConfirm" value="Confirm" onclick="return parseData();" id="MainContent_crawlConfirm">');
            $("#MainContent_crawlConfirm").focus();
            $("#MainContent_crawlConfirm").keyup(function () {
                parseData();
                return false;
            });
        ...
        url: urlPath, type: 'GET', success: function (res) {
            ...
            var head = $(res.responseText).find('tr').each(function (idx, item) {
                if (idx != 0 && multiResult) { // create list to choose from
                    ...
                    // remove hyperlinks
                    $('#row' + idx).find("a:first").replaceWith("<div class='clickable' onclick='searchResultSelect(\"row" + idx + "\")'>" +
                    ...
                    if (value.search("Name") >= 0 && value.search("Email") >= 0) {
                        multiResult = true;
                        $('#links').append("<table id='results' style='{list-style: none;}'></table>");
                    }
                    else {
                        data[data.length] = value;
                        rawData[rawData.length] = $(this).text().trim();
            ...
            if (!multiResult) {
                ...
                if ($("#results tr").length <= 0) {
                    ...
                    $('#links').prepend('<br/>');
                    $('#links').prepend($("#results tr").length + " results found.");
                }
            }
            var w = $("#modalDiv").width();
            $("#modalDiv").css("margin-left", w / -2);
...
$(document).ready(function () {
    $("#MainContent_FirstNameBox").focus(); // apply focus to the first name box   .
    // Check if U of M email box should be filled with email box content if it is a umn email address.
    $("#MainContent_EmailBox").blur(function () {
        var email = $("#MainContent_EmailBox").val();
        if (email.search("umn.edu") >= 0) {
            $("#MainContent_UEmailBox").val(email);
        }
    });
    // Handle enter press in the first name box for U-card swipe completion.
    $("#MainContent_FirstNameBox").keydown(function (event) {
        if (event.keyCode == 13) {
            //decode u-card
            var ucardRaw = $("#MainContent_FirstNameBox").val();
                ...
                for (i = 0; i < firstSplit.length; i++) {
                    ...
                    if (ind !== null && ind !== undefined && ind >= 0) {
                        ...
                        if (secondSplit !== null && secondSplit !== undefined && secondSplit.length == 2) {
                            ...
                            globLastName = lastName;
                            $("#MainContent_LastNameBox").val(lastName);
                            ...
                            if (thirdSplit !== null && thirdSplit !== undefined) {
                                ...
                                globFirstName = firstName;
                                $("#MainContent_FirstNameBox").val(firstName);
                            }
                        }

                        $("#MainContent_FirstNameBox").blur();
